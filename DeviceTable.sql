-- Device Table for T1 Device Rentals
CREATE TABLE Device (
  SerialNo VARCHAR(50) PRIMARY KEY,          -- Unique device serial number
  AssetNo VARCHAR(50) NOT NULL,               -- Asset tag/number for inventory tracking
  PONo VARCHAR(50),                           -- Purchase Order number
  DeliveryNo VARCHAR(50),                     -- Delivery note/reference
  DeviceType VARCHAR(50) NOT NULL,             -- e.g., Laptop, Tablet, Desktop
  isActive BOOLEAN NOT NULL DEFAULT TRUE,     -- Is the device active in the system?
  isFunctional BOOLEAN NOT NULL DEFAULT TRUE, -- Is the device working correctly?
  isDamaged BOOLEAN NOT NULL DEFAULT FALSE,   -- Has the device been damaged?
  ImageInfo TEXT,                             -- URL or description of the device image
  CurrentUser INT,                            -- FK: references StudentInfo or FacultyInfo (if linked)
  LoginInfo TEXT,                             -- Login credentials or notes
  isLost BOOLEAN NOT NULL DEFAULT FALSE,      -- Is the device reported as lost?

  -- Constraint to make sure DeviceType is either Laptop, Tablet, or Desktop
  CHECK (DeviceType IN ('Laptop', 'Tablet', 'Desktop'))
);

-- Insert 10 sample rows into Device
